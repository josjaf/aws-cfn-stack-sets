"""
export account_id_org_master=
export orgmasterlistaccountsrole=
export AWS_DEFAULT_PROFILE=shared
"""
...
def stack_set_deploy(stack_sets_boundary, shared_session):
    """
    ...
    for key, value in stack_sets_boundary.items():

        # try, catch operation in progress error and keep running
        try:
            stack_set_name = value['StackSet']['StackSetName']
            Helpers.print_separator(f"Deploying StackSet: {value['StackSet']['StackSetName']}")

            if not Cfn_helpers.stack_set_exists_check(shared_session, value['StackSet']['StackSetName']):
                print(f"Stack Set: {value['StackSet']['StackSetName']} CREATING")
                if create_update:
                    operation_response = cfn.create_stack_set(**value['StackSet'])
                    print(f"Operation Response: {operation_response}")
                    # Cfn_helpers.operation_id_waiter(session=shared_session,
                    #                                 stack_name=value['StackSet']['StackSetName'],
                    #                                 operation_response=operation_response)
                if deploy:
                    Cfn_helpers.stack_set_waiter(shared_session, stack_set_name)
                    print(f"Creating Stack Instances: {stack_set_name}")
                    response = cfn.create_stack_instances(**value['InstanceArgs'])
                    print(response)
                    Cfn_helpers.stack_set_waiter(shared_session, stack_set_name)

            else:
                print(f"Stack Set: {value['StackSet']['StackSetName']} UPDATING")
                # print(value)
                if create_update:
                    Cfn_helpers.stack_set_waiter(shared_session, stack_set_name)
                    operation_response = cfn.update_stack_set(**value['StackSet'])
                    print(f"Operation Response: {operation_response}")
                    # Cfn_helpers.operation_id_waiter(session=shared_session,
                    #                                 stack_name=value['StackSet']['StackSetName'],
                    #                                 operation_response=operation_response)
                if deploy:
                    ...
                    Cfn_helpers.stack_set_waiter(shared_session, stack_set_name)
                    response = cfn.update_stack_instances(**value['InstanceArgs'])
...
def main():
    ...
    stack_sets = [{'TemplateBody': Helpers.file_to_string('templates/role.yml'), 'StackSetName': 'aws-cfn-stack-sets-child1', 'Description': 'Role1'},
                  {'TemplateBody': Helpers.file_to_string('templates/role.yml'), 'StackSetName': 'aws-cfn-stack-sets-child2', 'Description': 'Role2'},]
    ...
    for stack_set in stack_sets:
        ...
        # print(stack_sets_boundary)shared_session
        #stack_set_deploy(stack_sets_boundary=stack_sets_boundary, shared_session=shared_session)
        proc = Process(target=stack_set_deploy, args=(stack_sets_boundary, shared_session_thread))
        procs.append(proc)
        proc.start()
